...
const Experience = () => {
    ...
    return (_jsxs("section", { id: "experience", className: "py-20 bg-background px-4 relative overflow-hidden", children: [_jsxs("div", { className: "absolute inset-0 opacity-20", children: [_jsx(motion.div, { className: "absolute top-0 left-1/4 w-2 h-full bg-gradient-to-b from-transparent via-accent to-transparent", animate: {
                            opacity: [0.2, 0.8, 0.2],
                            scaleY: [0.8, 1.2, 0.8]
                        }, transition: { duration: 4, repeat: Infinity } }), _jsx(motion.div, { className: "absolute top-0 right-1/3 w-1 h-full bg-gradient-to-b from-transparent via-primary to-transparent", animate: {
                            opacity: [0.8, 0.2, 0.8],
                            scaleY: [1.2, 0.8, 1.2]
                        }, transition: { duration: 5, repeat: Infinity } })] }), _jsx(motion.h2, { className: "text-center text-4xl md:text-5xl font-display font-bold uppercase tracking-widest text-accent relative z-10", initial: { opacity: 0, y: -20 }, whileInView: { opacity: 1, y: 0 }, viewport: { once: true }, transition: { duration: 0.6 }, children: _jsx(motion.span, { animate: {
                        textShadow: [
                            "0 0 10px rgba(var(--accent), 0.5)",
                            "0 0 20px rgba(var(--accent), 0.8)",
                            "0 0 10px rgba(var(--accent), 0.5)"
                        ]
                    }, transition: { duration: 2, repeat: Infinity }, children: "Experience" }) }), _jsxs("div", { className: "mt-12 max-w-4xl mx-auto space-y-12 relative z-10", ref: ref, children: [_jsx(motion.div, { className: "absolute left-8 top-0 w-1 bg-gradient-to-b from-accent via-primary to-accent rounded-full", initial: { height: 0 }, animate: isInView ? { height: "100%" } : { height: 0 }, transition: { duration: 1.5, ease: "easeInOut" } }), experiences.map((exp, idx) => (_jsxs(motion.div, { className: `${glass} relative pl-20 group`, initial: { opacity: 0, y: 50, x: -50 }, whileInView: { opacity: 1, y: 0, x: 0 }, viewport: { once: true, amount: 0.2 }, transition: { delay: idx * 0.2, type: "spring", stiffness: 60 }, whileHover: {
                            scale: 1.02,
